...
class Agent1(Agent):
    ...
    def step(self, chess_board, player, opponent):
        ...
        for move in valid_moves:
            ...
            score = self.minimax(board_copy, depth=3, maximizing=True, player=player, opponent=opponent,
                                 move=move)  # max = True or False??
    ...
    def board_score(self, board, player, opponent, board_length, move):
        ...
        for r, c in corners:
            if board[r, c] == player:
                score += 100000
            elif board[r, c] == opponent:  # elif or if??
                score -= 100000
            else:  # Empty corner, adjacency is risky        # elif or if??
                # check if we are on a tile adjacent to a corner
                # adjacent to top left -> hardcode instead of r,c?
                if (r, c) == (0, 0):
                    if board[r + 1, c] == player or board[r + 1, c + 1] == player or board[
                        r, c + 1] == player:  # x squares are the worst
                        score -= 5000
    ...
    def minimax(self, board, depth, maximizing, player, opponent, move):
        ...
        if maximizing:
            ...
            for move in valid_moves:
                ...
                execute_move(board_copy, move, player)
                value = self.minimax(board_copy, depth - 1, False, player, opponent, move)
            ...
            for move in valid_moves:
                ...
                value = self.minimax(board_copy, depth - 1, True, player, opponent, move)
